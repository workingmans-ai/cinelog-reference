...
export async function saveMovie(movie) {
  ...
  if (error) {
    console.error("[Supabase] Error saving movie:", error.message);
    throw error;
  }

  console.log("[Supabase] Saved movie:", movie.title);
  return data;
}
...
export async function saveRating(movieId, ratings, movieData) {
  ...
  if (error) {
    console.error("[Supabase] Error saving rating:", error.message);
    throw error;
  }

  console.log("[Supabase] Saved rating for movie:", movieId);
  return data;
}
...
export async function getRating(movieId) {
  ...
  if (error && error.code !== "PGRST116") {
    console.error("[Supabase] Error fetching rating:", error.message);
    throw error;
  }
...
export async function getAllRatings() {
  ...
  if (error) {
    console.error("[Supabase] Error fetching ratings:", error.message);
    throw error;
  }

  console.log("[Supabase] Loaded", data.length, "ratings from database");
...
export async function deleteRating(movieId) {
  ...
  if (error) {
    console.error("[Supabase] Error deleting rating:", error.message);
    throw error;
  }

  console.log("[Supabase] Deleted rating for movie:", movieId);
  return true;
}
